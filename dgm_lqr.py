...
class PDE_DGM_LQR(nn.Module):
    ...
    def _pde_residual(self, t, x, alpha_batch):
        ...
        drift  = torch.sum(u_x * (x @ self.H.T), dim=1)
        drift += torch.sum(u_x * (alpha_batch @ self.M_mat.T), dim=1)
        cost_x = torch.sum(x * (x @ self.C_mat.T), dim=1)
        cost_a = torch.sum(alpha_batch * (alpha_batch @ self.D_mat.T), dim=1)

        return u_t + 0.5 * trace + drift + cost_x + cost_a

    # ── fit() ─────────────────────────────────────────────────────────────────

    def fit(self, max_updates=3000, batch_size=512, lr=1e-3,
            log_every=200, pde_epochs=1000, ham_epochs=500, lr_a=None):
        self.riccati.solve()
        if self.mode == "fixed_control":
            ...
            self._fit_policy_iteration_2(max_updates, pde_epochs, ham_epochs,
                                         batch_size, lr, lr_a=lr_a or lr * 2)
        self.eval()
        return self
    ...
    def _fit_fixed(self, max_updates, batch_size, lr, log_every):
        ...
        for step in range(1, max_updates + 1):
            ...
            if step % log_every == 0 or step == 1:
                ...
                print(f"Step {step:4d} | loss={loss.item():.4e} | rel_err={rel_err:.4f}")
    ...
    def _fit_policy_iteration(self, n_iter, pde_epochs, ham_epochs, batch_size, lr):
        ...
        for iteration in range(1, n_iter + 1):
            ...
            self.pia_history["v_loss_final"].append(loss.item())
            ...
            self.pia_history["h_loss_final"].append(full_H.item())
    ...
    def _fit_policy_iteration_2(self, n_iter, pde_epochs, ham_epochs, batch_size, lr, lr_a=None):
        dev = next(self.parameters()).device
        lr_a = lr_a if lr_a is not None else lr * 2
        ...
        # Persistent optimizer: keep Adam momentum state across iterations
        v_opt = torch.optim.Adam(self.v_net.parameters(), lr=lr)

        # Best-model checkpoint: restore weights from the iteration with lowest v_error
        best_v_err   = float('inf')
        best_v_state = None
        best_a_state = None
        ...
        for iteration in range(1, n_iter + 1):
            ...
            # Reset lr each iteration but keep momentum (mirrors run_policy_iteration_2)
            for pg in v_opt.param_groups:
                pg['lr'] = lr
            v_sch = torch.optim.lr_scheduler.CosineAnnealingLR(
                v_opt, T_max=pde_epochs, eta_min=lr * 0.01)

            a_opt = torch.optim.Adam(self.a_net.parameters(), lr=lr_a)
            a_sch = torch.optim.lr_scheduler.CosineAnnealingLR(
                a_opt, T_max=ham_epochs, eta_min=lr_a * 0.01)
            ...
            self.pia_history["v_loss_final"].append(v_loss)
            ...
            self.pia_history["h_loss_final"].append(h_loss)
            ...
            if v_err < best_v_err:
                best_v_err   = v_err
                import copy
                best_v_state = copy.deepcopy(self.v_net.state_dict())
                best_a_state = copy.deepcopy(self.a_net.state_dict())
                print(f"  *** New best at iteration {iteration}: v_err={v_err:.4f} ***")

        # Restore best weights so .value() / .control() use the best iteration
        if best_v_state is not None:
            self.v_net.load_state_dict(best_v_state)
            self.a_net.load_state_dict(best_a_state)
            print(f"\nRestored best model (v_err={best_v_err:.4f})")
    ...
    def _make_lqr_obj(self):
        ...
        lqr.solve_riccati = lambda g: None
