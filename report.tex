...
\begin{itemize}
    ...
    \item \item \textbf{\texttt{solve\_riccati(time\_grid)}:} numerically integrates the Riccati ODE backwards in time using \texttt{scipy.integrate.solve\_ivp} with the RK45 method, with relative tolerance \texttt{rtol=1e-9} and absolute tolerance \texttt{atol=1e-12}.
...
\item \textbf{Hamiltonian Minimisation:} The Hamiltonian decreases steadily (becoming more negative, e.g., from $-5$ to $-10$). Note that during training only the $a$-dependent terms $(\partial_x v)^\top M a + a^\top D a$ contribute gradients; the remaining terms $(\partial_x v)^\top Hx + x^\top Cx$ are included in the reported value for consistency with the full Hamiltonian formula above.
\end{itemize}
