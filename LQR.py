...
# Setup Global LQR Parameters
H, M, C, D, R, T = [[0.1, 0.], [0., 0.1]], np.eye(2), np.eye(2), np.eye(2)*0.1, np.eye(2), 1.0
x0 = [[1.0, 1.0]]
lqr_sim = LQR(H, M, C, D, R, T, sigma=0.5)

# Solve the Riccati ODE once to provide the ground truth [cite: 39, 44]
lqr_sim.solve_riccati(torch.linspace(0, T, 100))
# Get the true value function v(0, x0) and optimal control a(0, x0) [cite: 37, 41]
...
# -------- Exercise 1.2 -----------
print("1.2: MC Convergence...")
# Error vs Number of Time Steps (Fixed M = 10^5)
N_steps_list = [1, 10, 50, 100, 500, 1000, 5000]
M_large = 100000 

errors_N = []
for n in N_steps_list:
    v_mc = run_monte_carlo_implicit(lqr_sim, x0, n, M_large)
    # Define error as relative difference 
    err = abs(v_true - v_mc) / abs(v_true)
    errors_N.append(err)

# Error vs Number of Samples (Fixed N = 5000)
M_samples_list = [10, 50, 100, 500, 1000, 5000, 10000, 50000, 100000]
N_large = 5000 

errors_M = []
for m in M_samples_list:
    v_mc = run_monte_carlo_implicit(lqr_sim, x0, N_large, m)
    err = abs(v_true - v_mc) / abs(v_true)
    errors_M.append(err)
...
plt.subplot(1, 2, 1)
plt.loglog(N_steps_list, errors_N, '-o', label='MC Error')
plt.loglog(N_steps_list, [errors_N[0]/n for n in N_steps_list], '--', alpha=0.5, label='O(Δt) Slope')
plt.title("Error vs Number of Time Steps (M=10^5)")
plt.xlabel("Number of Time Steps (N) [Log Scale]")
plt.ylabel("Relative Error [Log Scale]")
plt.grid(True, which="both", ls="-", alpha=0.2)
plt.legend()

# Plot 2: Sample Convergence
plt.subplot(1, 2, 2)
plt.loglog(M_samples_list, errors_M, '-s', color='orange', label='MC Error')
plt.loglog(M_samples_list, [errors_M[0]/np.sqrt(m/M_samples_list[0]) for m in M_samples_list], 
           '--', alpha=0.5, label='O(1/√M) Slope')
plt.title("Error vs Number of Monte Carlo Samples (N=5000)")
plt.xlabel("Number of Samples (M) [Log Scale]")
plt.ylabel("Relative Error [Log Scale]")
plt.grid(True, which="both", ls="-", alpha=0.2)
plt.legend()
...
alpha_const = torch.tensor([[1.0], [1.0]])
dgm_model = DGMNet()
optimizer = optim.Adam(dgm_model.parameters(), lr=1e-3)
loss_dgm, dgm_mc_errors = [], []
v_mc_const = run_monte_carlo_explicit(lqr_sim, x0, 100, 2000, alpha_val=[1.0, 1.0])



for epoch in range(1001):
    optimizer.zero_grad()
    t_int, x_int = torch.rand(512), torch.rand(512, 1, 2) * 6 - 3
    t_int.requires_grad_(True); x_int.requires_grad_(True)
    u = dgm_model(t_int, x_int)
    grad_u = torch.autograd.grad(u.sum(), [t_int, x_int], create_graph=True)
    u_t, u_x = grad_u[0].view(-1, 1), grad_u[1].view(-1, 2, 1)
    
    lap = sum(torch.autograd.grad(grad_u[1][:, i].sum(), x_int, create_graph=True)[0][:, i] for i in range(2)).view(-1, 1)
    drift = torch.bmm(u_x.transpose(1, 2), (torch.tensor(H).float() @ x_int.transpose(1, 2) + torch.tensor(M).float() @ alpha_const)).view(-1, 1)
    running = (torch.bmm(x_int, torch.tensor(C).float() @ x_int.transpose(1, 2)).view(-1, 1) + 
               (alpha_const.t() @ torch.tensor(D).float() @ alpha_const))
    loss_pde = torch.mean((u_t + 0.5 * (lqr_sim.sigma**2) * lap + drift + running)**2)
    
    t_b, x_b = torch.full((512,), T), torch.rand(512, 1, 2) * 6 - 3
    loss_b = torch.mean((dgm_model(t_b, x_b) - torch.bmm(x_b, torch.tensor(R).float() @ x_b.transpose(1, 2)).view(-1, 1))**2)
    
    (loss_pde + loss_b).backward(); optimizer.step()
    loss_dgm.append((loss_pde + loss_b).item())
    
    if epoch % 200 == 0:
        err = abs(dgm_model(torch.tensor([0.]), torch.tensor([x0])).item() - v_mc_const) / v_mc_const
        dgm_mc_errors.append(err)

plt.figure(figsize=(12, 4))
plt.subplot(1, 2, 1); plt.plot(loss_dgm); plt.yscale('log'); plt.title("DGM Loss")
plt.subplot(1, 2, 2); plt.plot(range(0, 1001, 200), dgm_mc_errors, '-o'); plt.title("DGM vs MC Error")
plt.show()
...
plt.figure()
plt.plot(range(1, 6), pia_results["v_err"], label="Value Function Error")
plt.plot(range(1, 6), pia_results["a_err"], label="Control (Action) Error")
plt.xlabel("PIA Iteration")
plt.ylabel("Relative Error")    
plt.yscale('log')
plt.title("Exercise 4.1: Policy Iteration Convergence")
plt.legend()
plt.show()